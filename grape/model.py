...
def get_attention_pad_mask_v2(seq_q):
    batch_size, len_q = seq_q.size()
    len_k = 1
    pad_attn_mask = torch.zeros((batch_size, len_q, len_k)).byte()
    return pad_attn_mask


def get_attn_subsequent_mask(seq):
    subsequence_mask = torch.triu(torch.ones(seq.size(0), seq.size(1), seq.size(1)), 1).byte()  # 生成一个上三角矩阵
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attention_mask, dec_enc_attention_mask):
        dec_outputs, dec_self_attention = self.dec_self_attention(dec_inputs, dec_inputs, dec_inputs, dec_self_attention_mask)

        dec_outputs, dec_enc_attention = self.dec_enc_attention(dec_outputs, enc_outputs, enc_outputs, dec_enc_attention_mask)
